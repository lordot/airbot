            ...
            steps {
                script {
                    ...
                    withKubeConfig([credentialsId: 'lke-configfile', serverUrl: 'https://06689cbd-962c-42c5-bb54-8bef03b752ae.eu-central-1.linodelke.net']) {
                        ...
                        sh "kubectl -n ${NAMESPACE} create secret docker-registry repo-secret   --docker-server=${REPO}   --docker-username=${CREDS_USR}  --docker-password=${CREDS_PSW} --validate=false | kubectl apply -f -"
